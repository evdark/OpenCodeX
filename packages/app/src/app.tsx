...
import { PromptLibraryProvider } from "@/context/prompt-library"
import { ProjectMemoryProvider } from "@/context/project-memory"
import { ProviderProfilesProvider } from "@/context/provider-profiles"
...
const LegacyHomeRoute = lazy(async () => {
  const module = await import("@/pages/home")
  return { default: module.LegacyHome }
})
const NewHomeRoute = lazy(async () => {
  const module = await import("@/pages/home")
  return { default: module.NewHome }
})
const SessionPageRoute = lazy(async () => {
  const module = await import("@/pages/session")
  return { default: module.SessionPage }
})
const TargetSessionRouteContentRoute = lazy(async () => {
  const module = await import("@/pages/session")
  return { default: module.TargetSessionRouteContent }
})
...
const SessionRoute = () => {
  ...
  // is replaced by a code-session draft at /new-session?draftId=…
  createEffect(() => {
    if (!settings.general.newLayoutDesigns()) return
    if (params.id || search.draftId) return
    if (!tabs.ready()) return
    const directory = sdk().directory || chatDirectory(serverSync().data.path)
    if (!directory) return
    ...
    <Show when={requireServerKey(params.serverKey)} keyed>
      <ServerSDKProvider server={conn}>
        <ServerSyncProvider server={conn}>
          <Suspense>
            <TargetSessionRouteContentRoute />
          </Suspense>
        </ServerSyncProvider>
      </ServerSDKProvider>
    </Show>
...
function ResolvedDraftRoute(props: { draft: DraftTab }) {
  ...
  const key = () => `${props.draft.server}\0${props.draft.directory}\0${props.draft.chat ? "chat" : "project"}`
