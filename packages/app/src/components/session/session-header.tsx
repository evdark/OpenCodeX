...
import { isChatTab, useTabs } from "@/context/tabs"
...
export function SessionHeader() {
  ...
  const chatSession = createMemo(() => {
    const route = layout.route()
    if (route.type === "draft") {
      return isChatTab(sessionTabs.store.find((tab) => tab.type === "draft" && tab.draftID === route.draftID))
    }
    if (route.type === "session") {
      return isChatTab(
        sessionTabs.store.find(
  ...
  const search = () => !chatSession() && settings.visibility.search()
  ...
  const v2ActionsState = createMemo<SessionHeaderV2ActionsState>(() => ({
    ...
    browserOpened: view().browser.opened(),
    onBrowserToggle: () => view().browser.toggle(),
    reviewLabel: language.t("command.review.toggle"),
    reviewKeybind: reviewTooltipKeybind(command),
    reviewVisible: isDesktop() && !chatSession(),
    reviewOpened: view().reviewPanel.opened(),
    onReviewToggle: () => view().reviewPanel.toggle(),
    codingTools: !chatSession(),
  }))
            ...
            <Show
              ...
              fallback={
                <div class="flex items-center gap-2">
                  <Show when={!chatSession() && projectDirectory()}>
...
function SessionHeaderV2Actions(props: { state: SessionHeaderV2ActionsState }) {
  return (
    <div class="flex items-center gap-2">
      ...
      <Show when={props.state.codingTools}>
        <TooltipV2
          class="shrink-0"
          placement="bottom"
          value={
        ...
        >
          <IconButtonV2
            ...
            aria-controls="terminal-panel"
            icon={<Icon size="small" name={props.state.terminalOpened ? "terminal-active" : "terminal"} />}
          />
        </TooltipV2>
        <TooltipV2
          class="shrink-0"
        ...
        >
          <IconButtonV2
            ...
            aria-controls="browser-preview"
            icon={<Icon size="small" name="window-cursor" />}
          />
        </TooltipV2>
