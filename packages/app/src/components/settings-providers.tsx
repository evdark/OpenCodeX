...
const SettingsProvidersContent: Component<{ onBack?: () => void }> = (props) => {
  ...
  const customProviderInitialValue = (providerID: string): CustomProviderInitialValue | undefined => {
    const provider = serverSync().data.config.provider?.[providerID]
    if (!provider || !isConfigCustom(providerID)) return
  ...
  const editCustomProvider = (providerID: string) => {
    const provider = customProviderInitialValue(providerID)
    if (!provider) return
    dialog.show(() => <DialogCustomProvider onBack={dialog.close} provider={provider} />)
  }
  ...
  const applyProfile = async (id: string) => {
    ...
    // Note: We can't easily auto-connect providers from a profile because 
    // it requires API keys which we don't store in the profile.
    // So "Apply Profile" primarily acts as a fast way to trim down active providers.
                  ...
                  <div class="group flex flex-wrap items-center justify-between gap-4 min-h-16 py-3 border-b border-border-weak-base last:border-none">
                    <div class="flex items-center gap-3 min-w-0">
                      ...
                      <span class="text-14-medium text-text-strong truncate">{item.name}</span>
                        <Tag>{type(item)}</Tag>
                      <Tag>{language.t("settings.providers.health.ready")}</Tag>
                    </div>
                    <Show
                    ...
                    >
                      <div class="flex items-center gap-1">
                        <Show when={isConfigCustom(item.id)}>
                          <Button size="large" variant="ghost" onClick={() => editCustomProvider(item.id)}>
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 min-h-16 py-3 border-b border-border-weak-base last:border-none">
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-x-3">
                      ...
                      <Tag>{language.t("settings.providers.health.notConfigured")}</Tag>
                    </div>
