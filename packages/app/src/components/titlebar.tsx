          ...
          {(_) => {
            ...
            createEffect(() => {
              ...
              if (route.type === "session") {
                ...
                const existing = tabs.store.find((item) => item.type === "session" && item.sessionId === sessionId)
                const next = { server: route.server ?? server.key, sessionId, chat: existing?.type === "session" ? existing.chat : undefined }
            ...
            const openNewChat = () => {
              ...
              const directory = chatDirectory(global.ensureServerCtx(server.current).sync.data.path)
              if (!directory) return
              tabs.newDraft({ server: server.key, directory, chat: true }, "")
            }

            const openNewCodeSession = () => {
              const route = layout.route()
              const activeSession = session()
              if (route.type === "session" && activeSession) {
                const existing = currentTab()
                // From a chat session the + button stays a pure chat shell.
                if (existing?.type === "session" && existing.chat) {
                  openNewChat()
                  return
                }
              ...
              if (activeTab?.type === "draft") {
                tabs.newDraft(
                  { server: activeTab.server, directory: activeTab.directory, chat: activeTab.chat },
                  "",
                )
                return
              }
            ...
            const openNewTab = openNewCodeSession
