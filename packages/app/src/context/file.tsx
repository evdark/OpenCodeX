...
import { createFileMutations } from "./file/mutations"
...
export const { use: useFile, provider: FileProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const mutations = () => createFileMutations(sdk().client)

    const rename = async (fromInput: string, toInput: string) => {
      const from = path.normalize(fromInput)
      const to = path.normalize(toInput)
      if (!from || !to || from === to) return from

      const result = await mutations().rename(from, to)
      const next = path.normalize(result.data?.path ?? to)
      refreshParents(from, next)

      const openTab = tabs.all().find((tab) => path.pathFromTab(tab) === from)
      if (openTab) {
        tabs.close(openTab)
        tabs.open(path.tab(next))
      }
    ...
    const remove = async (input: string) => {
      ...
      await mutations().remove(file)
      refreshParents(file)

      for (const tab of tabs.all()) {
        const tabPath = path.pathFromTab(tab)
        if (!tabPath) continue
        if (tabPath !== file && !tabPath.startsWith(file + "/")) continue
        tabs.close(tab)
      }
    ...
    const copy = async (input: string) => {
      ...
      const result = await mutations().copy(file)
      const next = path.normalize(result.data?.path ?? "")
      refreshParents(file, next)
      return next
    }
