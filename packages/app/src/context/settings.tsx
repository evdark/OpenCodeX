...
import { createOpenCodePlusSettingsController } from "@/context/opencode-plus-settings-controller"
import {
  defaultOpenCodePlusSettings,
  migrateOpenCodePlusSettingsState,
  type OpenCodePlusSettings,
} from "@/context/opencode-plus-settings"
...
export {
  ...
  type OpenCodePlusFeatureSettings,
  type OpenCodePlusPreset,
...
const defaultSettings: Settings = {
  ...
  opencodePlus: defaultOpenCodePlusSettings,
}
...
const SettingsContext = createSimpleContext({
  name: "Settings",
  gate: false,
  init: () => {
    const [store, setStore, _, ready] = persisted(
      { key: "settings.v3", migrate: migrateOpenCodePlusSettingsState },
    ...
    return {
      ...
      general: {
        ...
        showSystemTray: withFallback(() => store.general?.showSystemTray, defaultSettings.general.showSystemTray),
        setShowSystemTray(value: boolean) {
          setStore("general", "showSystemTray", value)
          if (typeof window !== "undefined" && "api" in window) {
            // @ts-expect-error Types are not universally available here
            window.api?.setTrayVisibility?.(value)
          }
        },
      ...
      opencodePlus: createOpenCodePlusSettingsController(
        store as Store<{ opencodePlus: OpenCodePlusSettings }>,
        setStore as unknown as SetStoreFunction<{ opencodePlus: OpenCodePlusSettings }>,
      ),
    }
  },
})

export const { use: useSettings, provider: SettingsProvider } = SettingsContext
