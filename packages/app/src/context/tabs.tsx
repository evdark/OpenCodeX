...
/** Pure chat (no project tooling) — draft or promoted session with chat flag. */
export function isChatTab(tab: Tab | undefined): boolean {
  return !!tab?.chat
}

export function findChatTab(
  tabs: readonly Tab[],
  input: { draftID?: string; server?: ServerConnection.Key; sessionId?: string },
) {
  if (input.draftID) {
    return tabs.find((tab): tab is DraftTab => tab.type === "draft" && tab.draftID === input.draftID)
  }
  if (input.server && input.sessionId) {
    return tabs.find(
      (tab): tab is SessionTab =>
        tab.type === "session" && tab.server === input.server && tab.sessionId === input.sessionId,
    )
  }
}
  ...
  init: () => {
    ...
    const actions = {
      ...
      promoteDraft(draftID: string, session: Omit<SessionTab, "type">) {
        ...
        const next = { type: "session" as const, chat: draft?.chat, ...session }
        void startTransition(() => {
          setStore(
            produce((tabs) => {
