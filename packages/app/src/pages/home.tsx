...
import { archiveHomeSession, deleteArchivedHomeSession } from "./home-session-archive"
import { shouldOpenSessionInBackground } from "./home-session-open"
import { showToast } from "@/utils/toast"
import { chatDirectory } from "@/utils/chat-directory"

const HOME_SESSION_LIMIT = 64
const HOME_SESSION_HEADER_STICKY_TOP = 12
const HOME_SESSION_HEADER_TEXT_HEIGHT = 16
const HOME_SESSION_HEADER_FADE_DISTANCE = 16
const HOME_SESSION_ARCHIVE_LIMIT = 200
const SHOW_HOME_SESSION_ARCHIVE = true
const HOME_ROW_LAYOUT =
  "flex min-w-0 w-full shrink-0 cursor-default items-center rounded-[8px] bg-transparent text-left transition-[background-color,color,box-shadow,transform] duration-[120ms] ease-[var(--v2-ease-standard)] focus-visible:outline-none focus-visible:[box-shadow:var(--v2-focus-ring)]"
...
const HOME_SEARCH_RESULT_ROW =
  "flex h-11 w-full shrink-0 cursor-default items-center gap-2 rounded-[8px] border-0 py-3 pl-4 pr-5 text-left transition-[background-color,box-shadow,transform] duration-[120ms] ease-[var(--v2-ease-standard)] hover:bg-v2-overlay-simple-overlay-hover focus-visible:bg-v2-overlay-simple-overlay-hover focus-visible:outline-none focus-visible:[box-shadow:var(--v2-focus-ring)]"
...
export function NewHome() {
  ...
  function openNewChat() {
    ...
    if (!conn) return
    const ctx = global.ensureServerCtx(conn)
    const directory = chatDirectory(ctx.sync.data.path)
    if (!directory) return
    tabs.newDraft({ server: ServerConnection.key(conn), directory, chat: true })
  }
  ...
  function openProjectArchive(conn: ServerConnection.Any, project: LocalProject) {
    const ctx = global.ensureServerCtx(conn)
    const serverKey = ServerConnection.key(conn)
    const children = (session: Pick<Session, "id" | "directory">) =>
      ctx.sdk.client.session
        .children({ sessionID: session.id, directory: session.directory })
        .then((result) => result.data ?? [])

    dialog.show(() => (
      <HomeSessionArchiveDialog
        project={project}
        server={serverKey}
        directories={directories(project)}
        load={(directory) =>
          ctx.sdk.client.experimental.session
            .list({ directory, roots: true, archived: true, limit: HOME_SESSION_ARCHIVE_LIMIT })
            .then((result) => result.data ?? [])
        }
        openSession={(session) => openSession(session)}
        restoreSession={(session) =>
          import("./home-session-archive").then(({ restoreArchivedHomeSession }) =>
            restoreArchivedHomeSession({
              server: serverKey,
              session,
              update: (value) =>
                ctx.sdk.client.session.update({
                  ...
                  // null clears archive; SDK types only list number.
                  time: value.time,
                } as never),
              onError: (error) =>
                showToast({
    ...
    await archiveHomeSession({
      ...
      update: (value) =>
        ctx.sdk.client.session.update({
          sessionID: value.sessionID,
          directory: value.directory,
          time: { archived: typeof value.time.archived === "number" ? value.time.archived : undefined },
        }),
      remove: () =>
        setStore(
          produce((draft) => {
    ...
    <div class="rounded-[10px] shadow-[var(--v2-elevation-raised)] m-2 min-h-0 lg:overflow-hidden bg-v2-background-bg-base self-stretch flex-1">
      <div class="mx-auto grid h-full w-full max-w-[1080px] grid-rows-[auto_minmax(0,1fr)_auto] gap-4 px-3 lg:grid-cols-[280px_minmax(0,720px)] lg:grid-rows-1 lg:gap-8 lg:px-6">
        <HomeProjectColumn
          ...
          openProjectArchive={openProjectArchive}
...
function HomeProjectColumn(props: {
  ...
  openProjectArchive: (server: ServerConnection.Any, project: LocalProject) => void
...
function HomeServerRow(props: {
  ...
  return (
    <div class="group/server relative flex h-8 min-w-0 items-center rounded-[8px]">
      <button
        type="button"
      ...
      >
        <span
          ...
          class="inline-flex -ml-0.5 -mr-1.5 size-5 shrink-0 items-center justify-center rounded-[7px] text-v2-icon-icon-muted"
          classList={{
      ...
      <For each={props.projects}>
        {(project) => (
          <HomeProjectRow
            ...
            openProjectArchive={props.openProjectArchive}
...
function HomeProjectRow(props: {
  ...
  const [state, setState] = createStore({ menuOpen: false })
  return (
    <div class="group/project relative flex h-8 min-w-0 items-center rounded-[8px]">
      <button
        type="button"
        ...
        >
          ...
          <MenuV2.Portal>
            <MenuV2.Content>
              ...
              <MenuV2.Item onSelect={() => props.openProjectArchive(props.server, props.project)}>
                {props.language.t("home.sessions.archive.open")}
              </MenuV2.Item>
  ...
  return (
    <div class="w-full">
      <div ref={root} data-component="home-session-search" class="relative z-30 w-full">
        ...
        <label class="relative z-20 flex h-10 w-full items-center gap-2 rounded-[12px] bg-v2-background-bg-layer-01 py-1 pl-3 pr-2 text-v2-icon-icon-muted shadow-[var(--v2-elevation-raised)] transition-[background-color,box-shadow] duration-[120ms] ease-[var(--v2-ease-standard)] hover:bg-v2-background-bg-layer-02 focus-within:bg-v2-background-bg-layer-02 focus-within:[box-shadow:var(--v2-focus-ring)]">
...
function HomeSessionRow(props: {
  ...
  return (
    <div
      class="group/session relative flex h-11 min-w-0 items-center rounded-[8px]"
...
function HomeSessionArchiveDialog(props: {
  project: LocalProject
  server: ServerConnection.Key
  directories: string[]
  load: (directory: string) => Promise<Session[]>
  ...
  const [sessions, { mutate }] = createResource(
    () => props.directories,
    (directories) => Promise.all(directories.map(props.load)).then((items) => archivedHomeSessions(items.flat())),
  )
  const items = () => sessions() ?? []
  const archivedAt = (session: Session) =>
    language.t("home.sessions.archive.archivedAt", {
      time:
        DateTime.fromMillis(session.time.archived ?? session.time.updated ?? session.time.created)
          .setLocale(language.intl())
          .toRelative() ?? "",
    })

  const remove = (session: Session) => {
    if (state.deleting[session.id] || state.restoring[session.id]) return
    setState("deleting", session.id, true)
    props
      .deleteSession(session)
      .then((ok) => {
        if (!ok) return
        mutate((current) => (current ?? []).filter((item) => item.id !== session.id))
      })
      .finally(() => setState("deleting", session.id, false))
  }

  const restore = (session: Session) => {
    if (state.deleting[session.id] || state.restoring[session.id]) return
    setState("restoring", session.id, true)
    props
      .restoreSession(session)
      .then(() => {
        mutate((current) => (current ?? []).filter((item) => item.id !== session.id))
        showToast({
          variant: "success",
          title: language.t("home.sessions.archive.restored"),
        })
      })
      .finally(() => setState("restoring", session.id, false))
  }

  const open = (session: Session) => {
    dialog.close()
    props.openSession(session)
  }

  return (
    <DialogV2 size="large" class="max-h-[min(680px,calc(100dvh-40px))]">
      <DialogHeader closeLabel={language.t("common.close")}>
        <DialogTitleGroup
          title={language.t("home.sessions.archive.title", { project: displayName(props.project) })}
          description={language.t("home.sessions.archive.description")}
        />
      </DialogHeader>
      <DialogBody class="min-w-0">
        <Show
          ...
          fallback={
            <div class="flex min-h-32 items-center justify-center gap-2 text-v2-text-text-muted [font-weight:440]">
              <Spinner class="size-4" />
        ...
        >
          <Show
            when={items().length > 0}
            fallback={
              <div class="flex min-h-32 items-center justify-center px-4 text-center text-v2-text-text-muted [font-weight:440]">
                {language.t("home.sessions.archive.empty")}
              </div>
            }
          >
            <ScrollView class="-mr-2 max-h-[min(420px,55vh)] pr-2">
              <div class="flex min-w-0 flex-col gap-1">
                <For each={items()}>
                  {(session) => {
                    const title = createMemo(() => sessionTitle(session.title) || session.id)
                    const deleting = () => !!state.deleting[session.id]
                    const restoring = () => !!state.restoring[session.id]
                    return (
                      <div class="flex min-w-0 items-center gap-2 rounded-[8px] bg-v2-background-bg-layer-01 px-2 py-2">
                        <button
                          type="button"
                        ...
                        >
                          ...
                          <div class="flex min-w-0 flex-1 flex-col gap-0.5">
                            <div class="min-w-0 truncate text-v2-text-text-base [font-weight:530]">{title()}</div>
                            <div class="min-w-0 truncate text-[12px] leading-4 text-v2-text-text-muted [font-weight:440]">
                              {archivedAt(session)}
                            </div>
                          </div>
                        </button>
                        <ButtonV2
                          variant={restoring() ? "loading" : "outline"}
                          size="small"
                          class="shrink-0"
                          disabled={deleting() || restoring()}
                          onClick={() => restore(session)}
                        >
                          {restoring() ? language.t("common.loading") : language.t("home.sessions.archive.restore")}
                        </ButtonV2>
                        <ButtonV2
                        ...
                        >
                          {deleting() ? language.t("common.deleting") : language.t("common.delete")}
                        </ButtonV2>
                      </div>
                    )
                  }}
...
function archivedHomeSessions(sessions: Session[]) {
  ...
  return sessions
    ...
    .sort(
      (a, b) =>
        (b.time.archived ?? b.time.updated ?? b.time.created) - (a.time.archived ?? a.time.updated ?? a.time.created),
    )
}
  ...
  return (
    <div class="flex min-w-0 flex-col gap-4">
      ...
      <div class="flex min-w-0 flex-col gap-px" aria-hidden="true">
        <For each={[0, 1, 2, 3]}>
          {() => (
            <div class="h-11 rounded-[10px] bg-v2-background-bg-layer-01 opacity-80 shadow-[var(--v2-elevation-raised)]" />
          )}
        </For>
      </div>
    </div>
