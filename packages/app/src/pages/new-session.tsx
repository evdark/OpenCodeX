...
export default function NewSessionPage() {
  ...
  const chat = createMemo(() => {
    if (!searchParams.draftId) return false
    return tabs.draft(searchParams.draftId).chat === true
  })
      ...
      <div class="flex-1 min-h-0 flex flex-col gap-2 p-2">
        <div class="@container relative flex flex-col min-h-0 h-full flex-1">
          <div class="flex-1 min-h-0 overflow-hidden rounded-[10px]">
            ...
            >
              <div class="relative size-full overflow-hidden bg-v2-background-bg-deep flex flex-col items-center justify-center px-6">
                <div class={NEW_SESSION_CONTENT_WIDTH + " flex flex-col items-center gap-6"}>
                  <div class="flex flex-col items-center gap-3 text-center">
                    <WordmarkV2 class="h-auto w-full max-w-xs text-v2-icon-icon-base" />
                    <div class="text-20-medium text-text-strong">{language.t("session.chat.title")}</div>
                    <div class="text-13-regular text-text-weak">{language.t("session.chat.subtitle")}</div>
                  </div>
                  <Show
                    when={prompt.ready() || promptReady()}
                    fallback={
