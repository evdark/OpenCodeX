...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const snapshotMemory = () => {
    if (!settings.opencodePlus.experimental.enabled() || !settings.opencodePlus.experimental.snapshotMemory()) return
    const sessionID = params.id
    if (!sessionID) return
    const messages = sync().data.message[sessionID] ?? []
    const title = sync().session.get(sessionID)?.title ?? sessionID
    const body = messages
      .slice(-12)
      .map((message) => {
        const parts = (sync().data.part[message.id] ?? []) as Array<{ type?: string; text?: string }>
        const text = parts
  ...
  const viewCmds = () => [
    ...
    viewCommand({
      id: "memory.snapshot",
      title: language.t("command.memory.snapshot"),
      description: language.t("command.memory.snapshot.description"),
      slash: "snapshot",
      disabled:
        !params.id ||
        !settings.opencodePlus.experimental.enabled() ||
        !settings.opencodePlus.experimental.snapshotMemory(),
      onSelect: snapshotMemory,
    }),
    viewCommand({
      id: "browser.openUrl",
      title: language.t("command.browser.openUrl"),
      description: language.t("command.browser.openUrl.description"),
      slash: "preview",
      onSelect: () => {
        const raw = window.prompt(language.t("browser.url.prompt"), browser.activeTab()?.input || "http://localhost:3000")
        if (!raw) return
