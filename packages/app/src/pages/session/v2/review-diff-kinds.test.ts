...
import { createReviewFileIndex, filterReviewFiles, reviewDiffKinds } from "./review-diff-kinds"
...
describe("filterReviewFiles", () => {
  ...
  test("reuses normalized file index for repeated filtering", () => {
    const files = ["src/Alpha.ts", "src/beta.ts", "lib/Gamma.ts"]
    const index = createReviewFileIndex(files)

    expect(filterReviewFiles(index, "SRC")).toEqual(["src/Alpha.ts", "src/beta.ts"])
    expect(filterReviewFiles(index, "gamma")).toEqual(["lib/Gamma.ts"])
    expect(filterReviewFiles(index, "")).toBe(files)
  })
})
