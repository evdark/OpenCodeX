...
export function ReviewPanelV2(props: ReviewPanelV2Props) {
  ...
  const diffByFile = createMemo(() => new Map(diffs().map((diff) => [diff.file, diff] as const)))
  const fileIndex = createMemo(() => createReviewFileIndex(diffs().map((diff) => diff.file)))
  const filteredFiles = createMemo(() => filterReviewFiles(fileIndex(), props.state.filter()))
  const filteredFileSet = createMemo(() => new Set(filteredFiles()))
  ...
  const activeDiff = createMemo(() => {
    ...
    if (focus && diffByFile().has(focus.file)) return focus.file
    const active = props.activeFile
    if (searching()) return active
    if (active && filteredFileSet().has(active)) return active
    return filteredFiles()[0]
  })
  const activeItem = createMemo(() => {
    const file = activeDiff()
    return file ? diffByFile().get(file) : undefined
  })
    ...
    <SessionReviewV2
      ...
      sidebar={
        ...
        <ReviewPanelV2Sidebar
          ...
          onSelectProjectFile={props.onSelectProjectFile}
          diffs={diffs}
    ...
    >
      ...
      >
        <Show
          ...
          fallback={
            <FileTreeV2
              allowed={props.diffs().length === 0 && props.state.filter().trim().length === 0 ? undefined : props.filteredFiles()}
              kinds={props.kinds()}
              draggable={false}
              active={props.activeDiff()}
              onFileClick={(node) => props.onSelectProjectFile?.(node.path) ?? props.onSelectFile(node.path)}
            />
          }
        >
          <Show
