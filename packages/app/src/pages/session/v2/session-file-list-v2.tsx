...
export function SessionFileListV2(props: {
  ...
  const indexByPath = createMemo(() => new Map(props.files.map((file, index) => [file, index] as const)))
  const indexByNormalizedPath = createMemo(
    () => new Map(normalized().map((file, index) => [file, index] as const)),
  )
  ...
  const virtualizer = createVirtualizer<HTMLDivElement, HTMLDivElement>({
    ...
    rangeExtractor: (range) => {
      ...
      const path = focused()
      const index = path ? (indexByPath().get(path) ?? -1) : -1
  ...
  createEffect(() => {
    const index = indexByNormalizedPath().get(highlighted()) ?? -1
    if (index < 0) return
    queueMicrotask(() => {
