...
const main = Effect.gen(function* () {
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "OpenCodeX Dev")
  ...
  app.setAsDefaultProtocolClient("opencodex")
  registerRendererProtocol()
  setDockIcon()
  
  // Try to load initial tray setting from store if possible, 
  // otherwise default to initTray and let the renderer manage it via IPC
  let shouldInitTray = true
  try {
    const store = require("./store").getStore("settings.v3")
    const settingsStr = store.get("settings.v3")
    if (settingsStr && typeof settingsStr === "string") {
      const settings = JSON.parse(settingsStr)
      if (settings?.general?.showSystemTray === false) {
        shouldInitTray = false
      }
    }
  } catch (error) {
    logger.warn("failed to read initial tray settings", error)
  }
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const ready = yield* awaitSidecarStartup({
      startup: Effect.promise(() =>
