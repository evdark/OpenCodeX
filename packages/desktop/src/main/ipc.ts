...
type Deps = {
  ...
  setTrayVisibility: (visible: boolean) => void
}
...
export function registerIpcHandlers(deps: Deps) {
  ...
  ipcMain.handle(
    "write-file",
    async (_event: IpcMainInvokeEvent, filePath: string, data: ArrayBuffer | Uint8Array | string) => {
  ...
  ipcMain.on("set-tray-visibility", (_event: IpcMainEvent, visible: boolean) => {
    deps.setTrayVisibility(visible)
  })
}
