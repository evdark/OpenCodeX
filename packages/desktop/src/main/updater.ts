...
export function setupAutoUpdater(stop: () => Promise<void>) {
  ...
  const feed = releaseFeed({
    ...
    owner: import.meta.env.OPENCODE_RELEASE_OWNER,
    repo: import.meta.env.OPENCODE_RELEASE_REPO,
  })
  autoUpdater.setFeedURL({ provider: "github", owner: feed.owner, repo: feed.repo, channel: "latest" })
  logger.log("auto updater configured", {
    channel: autoUpdater.channel,
    releaseOwner: feed.owner,
    releaseRepo: feed.repo,
