...
function registerWindow(win: BrowserWindow, id: string) {
  ...
  win.on("close", (event) => {
    // Hide to tray when tray is active and the app is not quitting.
    if (registry.isQuitting()) return
    // Lazy require avoids circular import with tray.ts
    const { isTrayActive } = require("./tray") as typeof import("./tray")
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("OpenCodeX is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
