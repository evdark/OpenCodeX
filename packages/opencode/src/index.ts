...
import {
  ...
  PlusSettingsCommand,
  PlusSetupCommand,
  PlusWorkspaceCommand,
} from "./cli/cmd/plus"

const args = hideBin(process.argv)
const argvBase = pathBaseName(process.argv[1] || "")
const isPlus =
  process.env.OPENCODEX === "1" ||
  process.env.OPENCODE_PLUS === "1" ||
  argvBase.includes("ocx") ||
  argvBase.includes("ocplus")
const scriptName = isPlus ? "ocx" : "opencode"
...
function show(out: string) {
  ...
  if (!text.startsWith("opencode ") && !text.startsWith("ocx ") && !text.startsWith("ocplus ")) {
