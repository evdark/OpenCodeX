...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setArchived = Effect.fn("Session.setArchived")(function* (input: { sessionID: SessionID; time?: number }) {
      // Omit/undefined time clears archive (restore); numbers set the archive stamp.
      const archived = input.time ?? (null as unknown as undefined)
      yield* patch(input.sessionID, {
        time: { archived, updated: Date.now() },
      }).pipe(Effect.orDie)
    })
