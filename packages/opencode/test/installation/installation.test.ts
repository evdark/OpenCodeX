...
describe("installation", () => {
  describe("latest", () => {
    ...
    testEffect(
      testLayer((request) => {
        ...
        return jsonResponse({ tag_name: "v1.2.4" })
      }),
    ).effect("reads OpenCode+ release versions from the fork repository", () =>
      Effect.gen(function* () {
        const result = yield* Installation.use.latest("curl")
        expect(result).toBe("1.2.4")
        expect(githubCalls).toContain("https://api.github.com/repos/evdark/opencode-plus/releases/latest")
      }),
    )
  ...
  describe("upgrade", () => {
    ...
    testEffect(
      testLayer(
        (request) => {
          expect(request.url).toBe("https://raw.githubusercontent.com/evdark/opencode-plus/dev/install")
