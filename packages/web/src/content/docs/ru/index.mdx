...
- **Менеджеры пакетов**
  ...
  > OpenCode+ does not publish fork-specific npm, pnpm, yarn, Bun, or Homebrew packages yet. Use the install script above or download from OpenCode+ releases.
...
- **Использование Mise**
  ...
  ```bash
  mise use -g github:evdark/opencode-plus
  ```

- **Использование Docker**

  ```bash
  docker run -it --rm ghcr.io/evdark/opencode-plus
  ```

For Windows, use WSL with the OpenCode+ install script for the most reliable setup.

Вы также можете получить бинарный файл в разделе [Releases](https://github.com/evdark/opencode-plus/releases).
